...
function RoomFormContent({
  ...
  editingRoom: Room | null | undefined;
  tiers: { id: string; name: string }[];
  onSubmit: (data: { name: string; description: string; type: Room['type']; allowedTierIds: string[] }) => Promise<void>;
  isSubmitting: boolean;
  onOpenChange: (open: boolean) => void;
}) {
  const [name, setName] = useState(editingRoom?.name ?? '');
  const [description, setDescription] = useState(editingRoom?.description ?? '');
  const [type, setType] = useState<Room['type']>(editingRoom?.type ?? 'general');
  const [allowedTierIds, setAllowedTierIds] = useState<string[]>(editingRoom?.allowedTierIds ?? []);
...
export function CreateRoomDialog({
  ...
  const formKey = open ? (editingRoom?.id ?? '__new__') : '__closed__';
