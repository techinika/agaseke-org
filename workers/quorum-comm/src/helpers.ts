  ...
  return new Response(JSON.stringify(data), {
    ...
    headers: {
      ...
      "Access-Control-Allow-Origin":
        env?.ALLOWED_ORIGIN || "https://quorum-org-app.vercel.app",
    },
  });
}
...
export async function getFirebaseAccessToken(
  ...
  if (!tokenRes.ok) {
    ...
    throw new Error(
      `Firebase token request failed: ${tokenRes.status} ${errText.slice(0, 300)}`,
    );
  }
...
function firestoreDocToData(
  ...
  for (const [key, field] of Object.entries(fields)) {
    if ("stringValue" in field) result[key] = field.stringValue;
      ...
      const vals =
        (field.arrayValue as { values?: Record<string, unknown>[] }).values ||
        [];
